...
def distance(data1, data2, parameters, model):
		...
		alldist = -1
		sig1 = numpy.sum([ data1[:,0], data1[:,2] ], axis=0 ) * 0.0285714857
		sig2 = numpy.sum([ data1[:,3], data1[:,4], data1[:,28] ], axis=0 ) * 0.0285714857
		sig3 = numpy.sum([ data1[:,6], data1[:,7], data1[:,29] ], axis=0 ) * 0.0285714857
		sig4 = numpy.sum([ data1[:,9], data1[:,10], data1[:,25] ], axis=0 ) * 0.0285714857
		sig5 = numpy.sum([ data1[:,12], data1[:,13] ], axis=0 ) * 0.0285714857
		sig6 = numpy.sum([ data1[:,15], data1[:,16] ], axis=0 ) * 0.0285714857
		sig7 = numpy.sum([ data1[:,18], data1[:,19] ], axis=0 ) * 0.0285714857
		sig8 = numpy.sum([ data1[:,21], data1[:,22], data1[:,24] ], axis=0 ) * 0.0285714857
