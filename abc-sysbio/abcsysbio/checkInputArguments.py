...
def checkInputABC(info_new , fname, custom_distance, design ):
	...
	for param in range(0,len(priors[mod])):
		if not len(priors[mod][param])==5:
			return False, "\nThe prior distribution of parameter "+repr(param+1)+" in model "+ModelName[mod]+" is wrong defined!\n"

		if not (priors[mod][param][0]==0 or priors[mod][param][0]==1 or priors[mod][param][0]==2 or priors[mod][param][0]==3 or priors[mod][param][0]==4 or priors[mod][param][0]==5 or priors[mod][param][0]==6):
		...
		if priors[mod][param][0]==4:
			if not (priors[mod][param][1]>=-20 or priors[mod][param][2]>=-20):
				return False, "\nThe mean or scale of the hierarchical prior distribution of parameter "+repr(param+1)+" in model "+ModelName[mod]+" is wrong defined!\n"

		if priors[mod][param][0]==6:
			if not priors[mod][param][1]<priors[mod][param][2]:
				return False, "\nThe range of the uniform prior distribution of parameter "+repr(param+1)+" in model "+ModelName[mod]+" is wrong defined!\n"
