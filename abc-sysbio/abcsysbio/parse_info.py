...
re_prior_hierarchical=re.compile('hierarchical')
re_prior_latent=re.compile('latent') 
re_prior_hvar=re.compile('hivar')
...
def process_prior( tmp ):
    ...
    if re_prior_const.match( tmp[0] ):
    ...
    elif re_prior_hierarchical.match( tmp[0] ):
        prior_tmp[0] = 4
        try:
            prior_tmp[1] = float( tmp[1] )
            prior_tmp[2] = float( tmp[2] )
            prior_tmp[3] = float( tmp[3] )
            prior_tmp[4] = float( tmp[4] )
        except:
            print "\nValue of the prior for model ", self.name[self.nmodels-1], "has the wrong format:", tmp[1]
            sys.exit()
            
    elif re_prior_latent.match( tmp[0] ):
        prior_tmp[0] = 5
        try:
            prior_tmp[1] = float( tmp[1] )
            prior_tmp[2] = float( tmp[2] )
    ...
    elif re_prior_hvar.match( tmp[0] ):
        prior_tmp[0] = 6
        try:
            prior_tmp[1] = float( tmp[1] )
            prior_tmp[2] = float( tmp[2] )
        ...
        print "\nSupplied parameter prior ", tmp[0], " unsupported", "re_prior_hvar ", re_prior_hvar
        sys.exit()
