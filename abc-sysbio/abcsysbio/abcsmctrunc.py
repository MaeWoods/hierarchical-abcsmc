...
priors: 
			  ...
			  4   ---   hierarchical distrbution.
			  			Example: Hyperparameters mu is uniform and sigma is fixed.            
						[4, 20, 1.5, 0.05]
						
			  5   ---   latent parameter.
			  			Example: Parameter K, calculated from hyperparameters, where x can be any number.            
						[5, k, x]
						
			  6   ---   Hyperparameter variance. 
			  			Example: Hyperparameters sigma is uniform.
			  			[6, 0, 10]
              Example:
        ...
        # self.kernels[i][2] is filled in during the kernelfn step and contains values/matrix etc depending ont he kernel
			...
			for i in range(self.nmodel):
				...
				for j in range(self.models[i].nparameters):
					if not(self.models[i].prior[j][0]==0 or self.models[i].prior[j][0]==5 ):
						ind.append(j)
			...
			if self.kernel_type == 1:
				...
				for m in range(self.nmodel):
					...
					for j in range(self.models[m].nparameters):
						if not (self.models[m].prior[j][0]==0 or self.models[m].prior[j][0]==2 or self.models[m].prior[j][0]==5):
							all_uniform = False
					if all_uniform == True:
...
#               print "this_model_parameters", this_model_parameters[1][1], "\n"
# 				print "length", self.models[ sampled_models[1] ].nparameters, "\n"
# 				print "length", self.models[ sampled_models[3] ].nparameters, "\n"
# 				print "this_model_parameters", this_model_parameters[1][2], "\n"
# 				print "this_model_parameters", this_model_parameters[1][3], "\n"
# 				print "this_model_parameters", this_model_parameters[2][1], "\n"
# 				print "this_model_parameters", this_model_parameters[3][1], "\n"
# 				print "this_model_parameters", this_model_parameters[1], "\n"
	
				#update_model_parameters = self.UpdateLatents(this_model_parameters, sampled_models)

				#print "update_model_parameters", update_model_parameters[1], "\n"
            ...
            while prior_prob <= 0 :
				...
				for n in range(self.models[ sampled_models[i] ].nparameters):
					if self.models[ sampled_models[i] ].prior[n][0] == 5: 
						...
						#print "pos", pos
						#print "var", var_n[pos]
						reti[n]=stats.truncnorm( (0 - mean_n[int(self.models[ sampled_models[i] ].prior[n][2])])/abs(var_n[int(self.models[ sampled_models[i] ].prior[n][2])]),(1000000 - mean_n[int(self.models[ sampled_models[i] ].prior[n][2])])/abs(var_n[int(self.models[ sampled_models[i] ].prior[n][2])]),loc=mean_n[int(self.models[ sampled_models[i] ].prior[n][2])],scale=abs(var_n[int(self.models[ sampled_models[i] ].prior[n][2])]) ).rvs(1)
						count = count + 1 
    ...
    def computeParticleWeights(self):
        ...
        for k in range(self.nparticles):
            ...
            for n in range(0,np):
                ...
                if self.models[ this_model ].prior[n][0]==5:
                    x=1
                                 
                if self.models[ this_model ].prior[n][0]==6:
                    x=statistics.getPdfUniform(self.models[ this_model ].prior[n][1],
                                               self.models[ this_model ].prior[n][2],
                                               this_param[n])                                                                 
                pprob = pprob*x
